...
const CashFlowStatement = () => {
  ...
  const [formData, setFormData] = useState({ companyName: DEFAULT_REPORT_COMPANY_NAME, period: "" });
  ...
  const saveSimpleStatement = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/cashflow-statement/simple/create`, {
        ...
        body: JSON.stringify({
          companyName: getReportCompanyName(simpleFormData.companyName),
      ...
      if (response.ok) {
        ...
        setSimpleFormData((prev) => ({ ...prev, period: "", inflowText: "", outflowText: "" }));
        setSimpleResult(null);
        fetchSimpleStatements();
      } else {
  ...
  const downloadSlip = (statement?: CashFlowStatement | SimpleCashFlowStatement, isSimple?: boolean) => {
    ...
    if (isSimple && statement) {
      ...
      dataToUse = {
        ...
        companyName: simpleStmt.companyName || simpleFormData.companyName,
    ...
    doc.text(getReportCompanyName(dataToUse.companyName || formData.companyName || simpleFormData.companyName), pageW / 2, 10, { align: "center" });
    doc.setFontSize(16);
            ...
            <Card className="liquid-panel overflow-hidden rounded-[36px] border-white/55 transition-all duration-500 bg-white/40">
              ...
              <CardContent className="space-y-6 p-8">
                <div className="space-y-3 group">
                  <Label className="font-semibold text-slate-700 text-lg">Enter Your Company Name</Label>
                  <Input
            ...
            <Card className="liquid-panel overflow-hidden rounded-[36px] border-white/55 transition-all duration-500 bg-white/40">
              ...
              <CardContent className="space-y-6 p-8">
                <div className="space-y-3">
                  ...
                  <Input
                    placeholder="Enter your company name"
                    value={simpleFormData.companyName}
                    onChange={(e) => handleSimpleInputChange("companyName", e.target.value)}
                    className="h-12 rounded-[18px] border-slate-200 bg-white/80 text-slate-900 placeholder:text-slate-400 focus:border-slate-300 focus:ring-0 transition-all duration-300"
                  />
                </div>
