...
// 🔒 POST route to add bookkeeping entry - BLOCKED for direct manual user inputs
router.post("/add", verifyToken, async (req, res) => {
    if (!req.body.isAutomated) {
        return res.status(403).json({
            message: "Direct user inputs are disabled in Bookkeeping. All financial entries MUST originate from Inventory or Invoice modules."
        });
    }
